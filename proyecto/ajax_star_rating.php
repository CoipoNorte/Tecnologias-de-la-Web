<?php
    ...
    $query = "SELECT COUNT(*) AS countProduct FROM ratio_producto WHERE id_producto = '" . $id_producto ."'". " AND user_id = " . $user_id;
    $result = mysqli_query($conn, $query);
    $getdata = mysqli_fetch_array($result);
    $count = $getdata['countProduct'];

    if($count == 0){
        $insertquery = "INSERT INTO ratio_producto(user_id, id_producto, rating) values(". $user_id .", '". $id_producto . "'". " , ". $rating .")";
        mysqli_query($conn, $insertquery) or die (mysqli_error($conn));
    }else{
        $updateRating = "UPDATE ratio_producto SET rating = " . $rating . " WHERE user_id = " . $user_id . " AND id_producto = '" . $id_producto . "' ";
